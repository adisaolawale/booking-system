...
// `testimonials` here is the featured subset, already fetched and ordered
// server-side — this component no longer knows or cares where it came from.
export function Testimonials({ testimonials }: { testimonials: Testimonial[] }) {
  const pages = useMemo(() => buildPages(testimonials), [testimonials]);
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl">
        ...
        >
          {pages[active].map((slot) =>
            slot === "cta" ? (
              <CTACard key="cta" />
            ) : (
              <TestimonialCard key={slot.id} testimonial={slot} />
            )
          )}
        </div>
